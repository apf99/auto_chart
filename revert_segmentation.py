...
if anomaly_start_columns:
    for region_index, (start_column, end_column) in enumerate(zip(anomaly_start_columns, anomaly_end_columns)):
        start_groups = anomaly_start_groups[region_index][0]  # Previous groups at start
        end_groups = anomaly_end_groups[region_index][1]  # Current groups at end

        # Identify the anomalous group index
        anomalous_group_index = next(i for i, (start, end) in enumerate(zip(start_groups, anomaly_start_groups[region_index][1]))
                                     if end[2] > start[2] * 1.8)

        # Extract start and end y-values for the anomalous group
        start_top_y, start_bottom_y, _ = start_groups[anomalous_group_index]
        end_top_y, end_bottom_y, _ = end_groups[anomalous_group_index]

        # Calculate gradients
        num_columns = end_column - start_column
        top_gradient = (end_top_y - start_top_y) / num_columns
        bottom_gradient = (end_bottom_y - start_bottom_y) / num_columns
        ...
        print(f"End y-range: {end_top_y} to {end_bottom_y}")
        print(f"Top gradient: {top_gradient}")
        print(f"Bottom gradient: {bottom_gradient}")

        print("\nProcessing columns:")

        # Iterate through the columns of the anomalous region
        for col in range(start_column, end_column + 1):
            # Calculate interpolated y-values
            steps = col - start_column
            interpolated_top_y = start_top_y + top_gradient * steps
            interpolated_bottom_y = start_bottom_y + bottom_gradient * steps

            # Round to nearest integer
            predicted_top_y = round(interpolated_top_y)
            predicted_bottom_y = round(interpolated_bottom_y)
            predicted_height = (max(predicted_bottom_y, predicted_top_y) - min(predicted_bottom_y, predicted_top_y)) + 1
...
if final_image is not None:
    ...
    print("\nPress 'q' to close the image and end the program.")
    while True:
