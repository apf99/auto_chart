...
def identify_pixel_groups(bitmap, column, min_height):
...
def initialize_curve_thicknesses(bitmap, start_points, end_points):
    ...
    for x in range(right_start, left_end, sample_step):
        group_count, groups = identify_pixel_groups(bitmap, x, 1)  # Use 1 as min_height initially
...
for x in range(left_x, right_x + 1):
    ...
    group_count, groups = identify_pixel_groups(bitmap, x, min_height)
    ...
    if group_count > len(start_points):
        ...
        print(f"Minimum of moving averages: {min_avg:.2f}")
        print(f"Minimum size threshold: {min_height}")
        print("Group sizes:")
        for i, (_, _, size) in enumerate(groups):
            print(f"  Group {i + 1}: {size}")
        print("--------------------")
